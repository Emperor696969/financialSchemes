// Initialize transactions from cookies or empty array
let transactions = [];
const cookies = document.cookie.split(';');
for (let cookie of cookies) {
    const [name, value] = cookie.split('=');
    if (name.trim() === 'transactions') {
...
function renderList() {
    ...
    transactions.forEach(({ id, name, amount, date, type }) => {
        ...
        li.innerHTML = `
            ...
            <div class="action">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" onClick="deleteTransaction(${id})"> <!-- Passing id to deleteTransaction function -->
...
function saveTransaction() {
    ...
    document.cookie = `transactions=${encodeURIComponent(JSON.stringify(transactions))};expires=${expires.toUTCString()}`;
}
